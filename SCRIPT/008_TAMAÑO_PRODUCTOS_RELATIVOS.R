...
base_productos <- read_excel("DATA/TAMANIO PRODUCTO/RELATIVOS/TOP 5 RELATIVOS.xlsx") %>% 
  clean_names() %>% 
  mutate(cpc = gsub("\\.","",as.character(cpc)),
         relativo_mar_24 = as.numeric(relativo_mar_24))
...
tam <- base_productos %>% 
  mutate(dominio = cpc,
         v_diseno = as.numeric(relativo_mar_24)) %>% 
  group_by(dominio) %>% 
  summarise(N = n(),
